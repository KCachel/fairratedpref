...
ggsave(fig_alt_tie, filename = glue::glue("plots/alt_tie_analysis.pdf"), device = cairo_pdf,
       width = pdfwidth, height = pdfheight, units = "in")

fig_skew_tie <- ggarrange(skew_fair, skew_wuloss,
                         ncol = 2, nrow = 1, common.legend = TRUE,legend = "top")

ggsave(fig_skew_tie, filename = glue::glue("plots/skew_tie_analysis.pdf"), device = cairo_pdf,
       width = pdfwidth, height = pdfheight, units = "in")
...
pdfwidth <- 10
pdfheight <- 2.5
ggsave(fairutil, filename = glue::glue("plots/datasets_results.pdf"), device = cairo_pdf,
       width = pdfwidth, height = pdfheight, units = "in")
...
ggsave(ggarrange(skew_top_ndkl, skew_alt_ndkl, skew_bot_ndkl, skew_top_wuloss, skew_alt_wuloss, skew_bot_wuloss,
                 ncol = 3, nrow = 2, common.legend = TRUE,legend = "top"), filename = glue::glue("plots/skew_tie_blocks.pdf"), device = cairo_pdf,
       width = pdfwidth, height = pdfheight, units = "in")

ggsave(ggarrange(alt_top_ndkl, alt_alt_ndkl, alt_bot_ndkl, alt_top_wuloss, alt_alt_wuloss, alt_bot_wuloss,
                 ncol = 3, nrow = 2, common.legend = TRUE,legend = "top"), filename = glue::glue("plots/alt_tie_blocks.pdf"), device = cairo_pdf,
       width = pdfwidth, height = pdfheight, units = "in")
